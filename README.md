...
1. Download installation.py from the repository.
2. Run the file, this will install everything.
3. To kill ollama on macOS, use this:
...
If you are trying to run the setup file on windows, please note that it should work for the most part, since it's a
python file. If anything breaks on other operating systems, please let me know.
...
calling and texting, set `Testing_automation` to `True` in `AssistantTools.py`. This will block texting and calling,
while allowing the LLM to still run the functions to experiment. If you wish to
...
- Keep at least 8-10 GB of free disk space for local model files.
